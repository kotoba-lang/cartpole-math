(ns kami.cartpole-math
  ...
  Kotoba\"). Unlike its sibling facade, this namespace had no `.kotoba` put in its
  place — it was simply removed, so the provider-side orphan check does not see it
  and only `verify-require-graph`-style reasoning about consumers does.

  `kotoba-lang/webgpu` requires `[kami.cartpole-math :as cm]` from BOTH
  `test/compute_golden_test.clj` and `scripts/gen_compute_golden.clj`, and pins
  this repo at `7c6cb307` — the migration commit ITSELF. So unlike
  `kotoba-lang/postfx`, whose consumer's stale pin was still holding it together,
  this consumer was already broken the day the migration landed: its golden test
  could not load. Nothing warned, because each repo's CI only looks at its own
  tree.

  `src/kotoba/cartpole_math.kotoba` remains the semantic authority for the physics
  and is not reverted. This file is restored VERBATIM from `7c6cb307^` — the
  `%.12g` serialisation and the SHA-256 in `output-hash` are what
  `kotoba-lang/webgpu`'s `fixtures/cartpole-compute-golden.json` pins, so changing
  a single operation here would silently invalidate a committed golden.

  `test/cartpole_math_parity_test.clj` asserts `step` agrees with the guest's four
  scalar exports. Two divergences are stated there rather than papered over:
  `output-bytes` / `output-hash` have NO guest counterpart (the guest has no bytes,
  no float formatting and no SHA-256), and agreement on the physics is to 1e-13,
  not bit-identity, because the guest uses bounded `f64-sin-bounded` /
  `f64-cos-bounded` and a different multiplication association than `Math/sin` and
  `(* pml theta-dot theta-dot sin-t)` here. That tolerance is this repo's own: its
  pre-existing `cartpole-math-test` already compares guest to a Clojure oracle at
  1.0e-13.
...
(defn output-bytes
  ...
  JVM ONLY. The `.cljc` extension is inherited from this file's history; the
  serialisation has always used `clojure.core/format` and the resulting bytes are
  what `kotoba-lang/webgpu`'s committed golden pins, so it is guarded rather than
  reimplemented for another host (ADR-2608133600). The physics above IS portable."
  [[x x-dot theta theta-dot :as state]]
  #?(:clj (.getBytes
            ...
            "UTF-8")
     :cljs (throw (ex-info "kami.cartpole-math/output-bytes is JVM-only"
                           {:state state}))))

(defn output-hash
  "SHA-256 hex of `output-bytes`. Deterministic across regenerations on the same
  input — the value the golden fixture pins. JVM only, for the same reason."
  [state]
  #?(:clj (let [md (java.security.MessageDigest/getInstance "SHA-256")
     ...
     :cljs (throw (ex-info "kami.cartpole-math/output-hash is JVM-only"
                           {:state state}))))
